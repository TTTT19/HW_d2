...
class db_work:
    ...
    def add_top_10_list(self, top_10_list):
        for top_10 in top_10_list:
            ...
            self.cur.execute(
                "INSERT INTO Users_match (id, first_name, last_name, bdate, city,relation,interests,match,groups_match,top3photo) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
                (top_10['id'], top_10['first_name'], top_10['last_name'], top_10['bdate'], top_10['city']['title'],
                 top_10['relation'], top_10['interests'], top_10['match'], top_10['groups_match'], top_10['top3photo']))
            self.conn.commit()
...
def vkinder():
    ...
    # 5) Сортируем по количеству совпадений по полю 'Match', пока не понимаю как это сделать

    list_for_sort = sorted(list_for_sort, key=lambda key: key['match'], reverse=True)

    # 6) Отбираем первые 50 аккаунтов в отсортированном списке

    top_20_list = list_for_sort[:20]
    ...
    # 8) Сортируем по количеству совпадений групп

    top_20_list = sorted(top_20_list, key=lambda key: key['groups_match'], reverse=True)

    # 9) Отбираем первые 10 аккаунтов в отсортированном списке

    top_10_list = top_20_list[:10]

    # 10) ищем профильные фотографии/ внутри сортируем по количеству лайков и оставляем первые 3
